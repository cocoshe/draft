...
class SpotifyClient:
    ...
    def get_top_tracks(self):
        response = requests.get(
            f'{self.base_url}/me/top/tracks', headers=self._auth_headers())
        response.raise_for_status()
        return response.json()
    
    def get_top_artists(self):
        response = requests.get(
            f'{self.base_url}/me/top/artists', headers=self._auth_headers())
        response.raise_for_status()
        return response.json()
    
    def get_recently_played(self):
        response = requests.get(
            f'{self.base_url}/me/player/recently-played', headers=self._auth_headers())
        response.raise_for_status()
        return response.json()
